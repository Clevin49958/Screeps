...
function autoDealExcess(terminal) {
    ...
    const orders = Game.market.getAllOrders({type: ORDER_BUY, resourceType: resourceType}).sort((a,b) => b.price - a.price);
    const minPrice = _.max([orders[0].price * 0.7, _.sum(orders.map(o => o.price)) / orders.length])
