    ...
    run: function(creep) {
        ...
        if (creep.memory.arrived == true){
            if ( creep.store.getCapacity(RESOURCE_ENERGY)>0 && creep.store.getFreeCapacity(RESOURCE_ENERGY) <=25){
                var container = creep.pos.findInRange(FIND_STRUCTURES, 0, {filter: s => s.structureType == STRUCTURE_CONTAINER});
            ...
            if(creep.pos.isNearTo(source)) {
                ...
                var container = creep.pos.findInRange(FIND_STRUCTURES,2, {filter: 
