...
module.exports = {
    ...
    harvest: function(creep){
        ...
        if (creep.pos.findInRange(FIND_STRUCTURES,0,{filter: (s)=>s.structureType == STRUCTURE_CONTAINER}).length>0){
    ...
    payTower: function(creep){
        var structure = creep.pos.findClosestByPath(FIND_MY_STRUCTURES,{filter: 
            (s) => s.structureType == STRUCTURE_TOWER && s.store.getFreeCapacity(RESOURCE_ENERGY)>100});
    ...
    withdrawContainer: function (creep) {
        var source = null;
        if (!source) {
            source = creep.pos.findClosestByPath(FIND_STRUCTURES, { filter: (s) => s.structureType == STRUCTURE_CONTAINER && s.store.getUsedCapacity(RESOURCE_ENERGY) > 200 });
        }
        if (source) {
            if (creep.withdraw(source, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                // move towards the source
