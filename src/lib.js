...
const { CARRY, HARVESTER, BUILDER, REPAIRER, WALL_REPAIRER, UPGRADER, roleNames, HARV_REMOTE } = require('./helper');
...
module.exports = {
    ...
    generateCreeps: function () {
        ...
        for (let name in Game.spawns){
            ...
            if (!Memory.creepTrack[spawn.name]) Memory.creepTrack[spawn.name] = {};
            var creepTrack = Memory.creepTrack[spawn.name];
            var res = undefined;
            ...
            if (Game.time % helper.logRate ==0) {
                ...
                [HARV_REMOTE,helper.CARRY, UPGRADER,BUILDER, REPAIRER, WALL_REPAIRER].forEach(role => {
                    console.log(`  ${role}: ${_.reduce(creepTrack, (acc, targetRoom) => acc+targetRoom[role], 0)}/`
            ...
            for (let targetRoom of Memory.myRooms[room]){
                ...
                for (let r of [UPGRADER,REPAIRER,BUILDER,WALL_REPAIRER]){
                    ...
                    if (creepTrack[targetRoom][r]<spawn.memory[targetRoom][r]) {
                        ...
                        if (Game.time%helper.logRate==0) console.log(`    Demand: ${targetRoom} ${r} Result: ${res}, have: ${creepTrack[targetRoom][r]}, need: ${spawn.memory[targetRoom][r]}`);
                        return;
                    }
                }
            };   
